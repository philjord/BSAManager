...
public class HashCode implements Comparable<HashCode> {
	...
	public HashCode(String hashName, boolean isFolder) {
		hash = hashCode(hashName, isFolder);
	}
	...
	/**
	 * DO NOT create a HashCode only to ask for getHash use the static method hashCode above
	 * @return
	 */
	public long getHash() {
		return hash;
	...
	public String toString() {
		return String.format("%08X-%08X",
				new Object[] {Integer.valueOf((int)(hash >>> 32)), Integer.valueOf((int)(hash & -1L))});
	}

	/**
	 * https://github.com/fo76utils/fo76utils/blob/main/libfo76utils/src/ba2file.cpp#L16
	 * https://github.com/fo76utils/fo76utils/blob/main/libfo76utils/src/common.cpp#L219
	 * @param hashName
	 * @param isFolder
	 * @return
	 */
	public static long hashCode(String hashName, boolean isFolder) {
		String name = null;
		String ext = null;
		if (isFolder) {
			//This is an odd one, the isFolder simply allows a shortcut of the call lastIndexOf, which might be worth it
			name = hashName;
		} else {
		...
		if (ext != null && ext.length() > 0) {
			...
			if (ext.equals(".nif"))
				...
				hash |= 0x80000000L;
			//else if (ext.equals(".bgsm"))
			//	hash |= 0x80000000L;
			//else if (ext.equals(".bgem"))
			//	hash |= 0x80000000L;
			// TODO: really should include these two  
			//|| ext.equals(".ktx"))
			//|| ext.equals(".astc"))		 
	...
	//https://github.com/Ryan-rsm-McKenzie/bsa/blob/master/src/bsa/fo4.cpp
	public static long hashCode2(String hashName, boolean isFolder) {
		String name = null;
		String ext = null;
		if (isFolder) {
			//This is an odd one, the isFolder simply allows a shortcut of the call lastIndexOf, which might be worth it
			...
			if (sep < 0) {
				...
				ext = hashName;//jebus this isn't going to work
			} else {
				name = hashName.substring(0, sep);
				ext = hashName.substring(sep + 1);// don't include the dot
			}
		}
	...
	//https://github.com/fo76utils/fo76utils/blob/main/libfo76utils/src/common.hpp#L175
	public static long hashCode3(String hashName, boolean isFolder) {
		//BA2File::hashFunction		
		int l = hashName.length();
		byte[] buffer = PrimitiveBytes.getBytesFast(hashName);
		long hash = hashFunctionUInt32(buffer, l);
		hash = hash | (l << 32);

		return hash;
	}

	private static long hashFunctionUInt32(byte[] buffer, int nBytes) {
		long h = 0xFFFFFFFFL;
		//byte[] q = buffer;
		//byte endp = buffer[nBytes - 1];
		if (nBytes < 8) {
			long tmp = 0;
			switch (nBytes) {
				case 1:
					...
					break;
				case 2:
					tmp = getUShort(buffer, 0);
					break;
				case 3:
					tmp = getUShort(buffer, 0) |  ((buffer[2] & 0xff) << 16);
					break;
				case 4:
					tmp = getUInteger(buffer, 0);
					break;
				case 5:
					tmp = getUInteger(buffer, 0) | ( (buffer[5] & 0xff) << 32);
					break;
				case 6:
					tmp = getUInteger(buffer, 0) | (getUShort(buffer, 4) << 32);
					break;
				case 7:
					tmp = getUInteger(buffer, 0) | ((getUInteger(buffer, 3) >> 8) << 32);
					break;
				default:
					return h;
			}
		...
		int q = 0;// q is an index into the buffer
		for (; (q + 8) < nBytes; q = q + 8) {
			hashFunctionUInt64(h, getLong(buffer, q));
		}
		long tmp = getLong(buffer, nBytes - 8);
		tmp = tmp >> ((q - (nBytes - 8)) << 3);
		hashFunctionUInt64(h, tmp);
		return h;
	}

	private static long hashFunctionUInt64(long h, long m) {
		long multValue = 0xEE088D97L;
		h = ((h ^ m) & 0xFFFFFFFFL) * multValue;		//^ => bitwise XOR
		h = h + (h >> 32);
		h = ((h ^ (m >> 32)) & 0xFFFFFFFFL) * multValue;
		h = h + (h >> 32);
		return h;
	}

	/*
	private static long hashFunctionCRC32(long h, byte c)
	{

	  h = (h >> 8) ^ crc32Table_EDB88320[(int)((h ^ (c & 0xff)) & 0xFF)];
	  return h;
	}
	
	private static long[] crc32Table_EDB88320 = new long[]
	{
	...
	protected static int getUShort(byte buffer[], int offset) {
		return (int)((buffer[offset + 0] & 0xff | (buffer[offset + 1] & 0xff) << 8) & 0xffffffffL);
	}
