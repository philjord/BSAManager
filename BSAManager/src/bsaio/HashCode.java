...
public class HashCode implements Comparable<HashCode> {
	...
	/**
	 * for use with bsa files, ba2 use hashCodeCRC32 below
	...
	public static long hashCode(String hashName, boolean isFolder) {
		...
		if (ext != null && ext.length() > 0) {
			...
			if (ext.equals(".nif"))
				...
				hash |= 128L;
			else if (ext.equals(".dds"))// ktx keeps the same mask, can't mix types
				hash |= 32896L;
	...
	/**
	 * For use with Ba2 BDTX format files //https://github.com/Ryan-rsm-McKenzie/bsa/blob/master/src/bsa/fo4.cpp
	 */
	public static long hashCodeCRC32(String hashName, boolean isFolder) {
		String name = null;
		String ext = null;
		if (isFolder) {
			name = hashName;
